...
FORWARD _PROTOTYPE( int set_group, (message *m_ptr) );
FORWARD _PROTOTYPE( int get_group, (message *m_ptr) );
FORWARD _PROTOTYPE( int set_schedule_a, (message *m_ptr) );
FORWARD _PROTOTYPE( int set_schedule_b, (message *m_ptr) );
FORWARD _PROTOTYPE( int get_time, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETGROUP:	r = set_group(&m);	break;
	    case SYS_GETGROUP:	r = get_group(&m);	break;
	    case SYS_SETSCHUA:	r = set_schedule_a(&m);	break;
	    case SYS_SETSCHUB:	r = set_schedule_b(&m);	break;
	    case SYS_GETTIME:	r = get_time(&m);	break;
	    default:		r = E_BAD_FCN;
	}
 ...
 *				set_group				     *
 *===========================================================================*/
PUBLIC int set_group(message *m_ptr) {
	struct proc* prc;
	int pid = m_ptr->m1_i1;
	int newgroup = m_ptr->m1_i2;
	
	if (newgroup < 0 || newgroup > 2)
		return(ESRCH);
	
	for (prc=BEG_PROC_ADDR;prc<END_PROC_ADDR;prc++) {
		if (istaskp(prc) ||  isservp(prc))
			continue; 
		if (prc->p_pid == pid) {
			prc->group = newgroup;
			return(OK);
 ...
 *				get_group				     *
 *===========================================================================*/
PUBLIC int get_group(message *m_ptr) {
	struct proc* prc;
	int pid = m_ptr->m1_i1;
 ...
 *				set_schedule_a			     *
 *===========================================================================*/
PUBLIC int set_schedule_a(message *m_ptr) {
	int a = m_ptr->m1_i1;
	int b = time_group[1];
	struct proc* prc;
	prc = proc_addr(m_ptr->PROC1);
 ...
 *				set_schedule_b			     *
 *===========================================================================*/
PUBLIC int set_schedule_b(message *m_ptr) {
	int a = time_group[0];
	int b = m_ptr->m1_i1;
	struct proc* prc;
	prc = proc_addr(m_ptr->PROC1);
 ...
 *				get_time			     *
 *===========================================================================*/
PUBLIC int get_time(message *m_ptr) {
